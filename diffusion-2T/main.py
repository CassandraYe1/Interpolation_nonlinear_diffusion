...
if __name__ == "__main__":
    ...
    [model_E, model_T] = train_model_reg(model_E, model_T, cfg=cfg, Nfit=cfg.Nfit_reg, lr_E=cfg.lr_E_reg, lr_T=cfg.lr_T_reg, epo=cfg.epoch_reg)
    end_time = time.time()
    ...
    model_E_cur = DeepNN(cfg=cfg).cuda()
    model_E_cur.load_state_dict(copy.deepcopy(model_E.state_dict()))
    model_T_cur = DeepNN(cfg=cfg).cuda()
    model_T_cur.load_state_dict(copy.deepcopy(model_T.state_dict()))

    # 训练并计时 | Train and time
    start_time = time.time()
    [model_E_cur, model_T_cur] = train_model_pde(model_E_cur, model_T_cur, cfg=cfg, Nfit=cfg.Nfit_pde, lr_E=cfg.lr_E_pde, lr_T=cfg.lr_T_pde, epo=cfg.epoch_pde)
    end_time = time.time()
    ...
    # === 可视化 === | === Visualization ===
    plot_E(E_reg, E_pinn, E_ref, X, Y, data_path, vmax=cfg.vmax_E)
    plot_T(T_reg, T_pinn, T_ref, X, Y, data_path, vmax=cfg.vmax_T)
